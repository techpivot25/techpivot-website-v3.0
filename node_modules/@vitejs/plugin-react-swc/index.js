...
const react = (_options) => {
	...
	return [
		...
		},
		{
			...
			transformIndexHtml: {
				handler() {
					if (!hmrDisabled && isBundledDev) return [{
						tag: "script",
						attrs: { type: "module" },
		...
		},
		{
			...
			configResolved(config) {
				...
				if (usingRolldown && !options.plugins && !options.useAtYourOwnRisk_mutateSwcOptions && !options.disableOxcRecommendation) config.logger.warn("[vite:react-swc] We recommend switching to `@vitejs/plugin-react` for improved performance as no swc plugins are used. More information at https://vite.dev/rolldown");
			},
			...
			async transform(code, id, transformOptions) {
				...
				const result = await transformWithOptions(file, code, options.devTarget, options, viteCacheRoot, {
					refresh,
					development: true,
					runtime: "automatic",
				...
				return {
					code: addRefreshWrapper(result.output.code, "@vitejs/plugin-react-swc", id, options.reactRefreshHost) ?? result.output.code,
		...
		options.plugins || options.useAtYourOwnRisk_mutateSwcOptions ? {
			...
			async transform(code, _id) {
				return (await transformWithOptions(_id.split("?")[0], code, "esnext", options, viteCacheRoot, {
					runtime: "automatic",
					importSource: options.jsxImportSource
				}))?.output;
			}
...
const transformWithOptions = async (id, code, target, options, viteCacheRoot, reactConfig) => {
	...
	try {
		...
		return {
			output: await transform(code, swcOptions),
			jsx: parser.syntax === "typescript" && parser.tsx || parser.syntax === "ecmascript" && parser.jsx
		};
	} catch (e) {
