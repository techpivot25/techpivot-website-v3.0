...
const Consultancy = () => {
  return (
    <ServicePageLayout
      ...
      metaTitle="IT & Software Consulting Services | Digital Strategy | TechPivot"
      metaDescription="Technology consulting services: digital transformation strategy, solution architecture, cloud and AI roadmaps, agile coaching and vendor evaluation. Book a call."
      keywords="IT consulting services, software consulting company, digital transformation consulting, technology strategy consulting, solution architecture consulting, cloud consulting services, AI consulting, agile transformation consulting"
    ...
    >
      ...
      {/* Our Approach */}
      <section className="py-20 lg:py-28 bg-surface-dark animate-section">
        <div className="container px-6 lg:px-12">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {approach.map((step, index) => (
                <AnimatedSection key={step.step} animation="fadeUp" delay={index * 100}>
                  ...
                  >
                    <div className="text-4xl font-bold text-surface-dark-foreground/90 mb-3">{step.step}</div>
                    <h3 className="font-bold text-surface-dark-foreground uppercase tracking-wide mb-3">{step.title}</h3>
                    <p className="text-sm text-surface-dark-foreground/60 leading-relaxed">{step.description}</p>
                  </motion.div>
                </AnimatedSection>
              ))}
            </div>
                ...
                {approach.map((step, index) => (
                  <AnimatedSection key={`circle-${step.step}`} animation="scaleIn" delay={index * 150 + 400}>
                    <div className="flex flex-col items-center">
                      ...
                      {/* Circle */}
                      <motion.div 
                        className="w-12 h-12 rounded-full border-2 border-primary bg-surface-dark flex items-center justify-center"
