...
const WebDevelopment = () => {
  return (
    <ServicePageLayout
      ...
      metaTitle="Web Development Services | React & Next.js Experts | TechPivot"
      metaDescription="Custom web development services using React, Next.js and Node.js. Fast, responsive, SEO-friendly websites and web apps built for growth. Get a quote today."
      keywords="web development services, custom website development, React development company, Next.js development, front end development services, full stack web development, responsive web design, enterprise web application development"
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute ${isTop ? 'bottom-20' : 'top-20'} w-44`}>
                      <motion.div
                        className="bg-surface-dark-foreground/10 backdrop-blur-sm border border-surface-dark-foreground/20 rounded-xl p-4 transition-all duration-300 hover:bg-surface-dark-foreground/15 hover:border-primary/50 hover:-translate-y-1 group cursor-default"
                        whileHover={{ scale: 1.02 }}
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <Icon className="w-5 h-5 text-primary flex-shrink-0 transition-transform duration-300 group-hover:scale-110" />
                          <h3 className="font-semibold text-sm text-surface-dark-foreground">{step.title}</h3>
                        </div>
                        <p className="text-xs text-surface-dark-foreground/60 leading-relaxed">{step.description}</p>
                      </motion.div>
                      {/* Connector Line */}
                    ...
                    {/* Number Circle */}
                    <motion.div
                      className="relative z-10 w-12 h-12 rounded-full bg-surface-dark border-2 border-primary flex items-center justify-center transition-all duration-300 hover:bg-primary group cursor-default"
            ...
            <div className="space-y-8">
              {process.map((step, index) => {
                ...
                return (
                  ...
                  >
                    {/* Number Circle */}
                    <div className="relative z-10 flex-shrink-0 w-10 h-10 rounded-full bg-surface-dark border-2 border-primary flex items-center justify-center">
                      <span className="text-sm font-bold text-primary">{step.step}</span>
                    </div>
                    
                    {/* Card */}
                    <div className="flex-1 bg-surface-dark-foreground/10 backdrop-blur-sm border border-surface-dark-foreground/20 rounded-xl p-4 transition-all duration-300 hover:bg-surface-dark-foreground/15 hover:border-primary/50 group">
                      <div className="flex items-center gap-2 mb-2">
                        <Icon className="w-5 h-5 text-primary" />
                        <h3 className="font-semibold text-surface-dark-foreground">{step.title}</h3>
                      </div>
                      <p className="text-sm text-surface-dark-foreground/60">{step.description}</p>
                    </div>
                  </motion.div>
                );
              })}
